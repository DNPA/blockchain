...
#define MAX_BITCOIN_ADDRESSES 65000000 // 60 million unique addresses.

#define MAX_TOTAL_TRANSACTIONS 75000000 // 70 million transactions.
...
//		printf("Gathering bitcoin addresses relative to this date: %s\r\n", getTimeString(refTime));
	...
	void saveStatistics(bool record_addresses,float minBalance)
	{
		...
		if ( plotCount )
		{
			...
			{
				fprintf(topKeys,"\r\n");
				fprintf(topKeys,"\"Scatter Plot Data values of %s bitcoin address balances with over %0.4f btc and number of days since last transaction. Sorted by Age\"\r\n", formatNumber(plotCount), minBalance);
...
//						const char *begin = scan;
	...
	virtual void reportTransactionValues(uint32_t date)
	{
		{
			char scratch[512];
			sprintf(scratch,"TransactionValues-%s.csv", getDateString(date));
